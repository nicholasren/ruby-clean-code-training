...
describe 'When a site has a contact' do
  before do
    ...
    @site = JobSite.new(double(:location), @contact)
  end
  ...
  describe '#email_contact' do
    it "sends an email to the contact" do
      email = double('email')
      email.should_receive(:deliver).with('John Smith').once
...
describe 'When a site lacks a contact' do
  before do
    @site = JobSite.new(double(:location), nil)
  end
  ...
  describe '#email_contact' do
    it "does not email the contact" do
      email = double('email')
      email.should_receive(:deliver).never
